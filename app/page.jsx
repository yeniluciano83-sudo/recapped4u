...
const TIERS = [
  ...
  { id: "standard", name: "Classic", price: "$35", tagline: "Everything you need, nothing extra",
    features: ["Unlimited photo & video uploads", "Shareable + printable QR code & link", "48-hour upload window after your event", "Curated photo gallery", "One recap video", "Choose your editing style", "Digital delivery", "Downloadable gallery for 2 months"] },
     ...
     <div style={{ ...cardStyle, marginTop: 16, border: "1.5px solid #C97A3D" }}>
      ...
      <Section id="demo" refs={sectionRefs} title="See a Sample Recap" icon={<Play size={20} color="#C97A3D" />}>
        <div style={{ ...cardStyle, textAlign: "center", padding: "48px 24px" }}>
          <div style={{ width: 60, height: 60, borderRadius: "50%", background: "#FBEEE0", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 18px" }}>
            <Play size={24} color="#C97A3D" fill="#C97A3D" style={{ marginLeft: 3 }} />
          </div>
          <p style={{ fontSize: 14, color: "#4a4642", margin: 0 }}>Sample recap coming soon — check back shortly.</p>
        </div>
          ...
          {FAQS.map((f, i) => (
            <div key={i} style={{ ...cardStyle, padding: 0 }}>
              <button onClick={() => setOpenFaq(openFaq === i ? null : i)}
                style={{ width: "100%", textAlign: "left", background: "none", border: "none", color: "#211F1D", padding: 18, fontSize: 14.5, fontWeight: 600, cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                {f.q}
                <ChevronRight size={16} style={{ transform: openFaq === i ? "rotate(90deg)" : "none", transition: "transform 0.2s", flexShrink: 0, marginLeft: 10 }} />
              </button>
              {openFaq === i && <p style={{ padding: "0 18px 18px", fontSize: 13.5, color: "#4a4642", lineHeight: 1.6, margin: 0 }}>{f.a}</p>}
            </div>
          ))}
        </div>
      </Section>
      ...
      <Section id="about" refs={sectionRefs} title="About" icon={<Star size={20} color="#C97A3D" />}>
        <p style={{ fontSize: 14.5, color: "#4a4642", lineHeight: 1.7, maxWidth: 600, marginBottom: 16 }}>
        ...
        <p style={{ fontSize: 14.5, color: "#4a4642", lineHeight: 1.7, maxWidth: 600 }}>
      ...
      <Section id="contact" refs={sectionRefs} title="Get In Touch" icon={<Mail size={20} color="#C97A3D" />}>
        ...
        <p style={{ fontSize: 12.5, color: "#8a857d", marginTop: 16 }}>
          <span aria-hidden="true">📱</span> Or reach us on WhatsApp (text only): <a href="https://wa.me/16465129151" target="_blank" rel="noopener noreferrer" style={{ color: "#C97A3D", fontWeight: 600, textDecoration: "none" }}>+1 (646) 512-9151</a>
        </p>
      </Section>
      </main>
