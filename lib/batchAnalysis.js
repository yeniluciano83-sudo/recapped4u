...
//
// Explicitly calls out ordinary swimwear as NOT flaggable -- pool parties,
// beach weddings, and "Vacation" bookings (a real event type this app
// supports, see EVENT_TYPE_MOOD in lib/inviteMoods.js) routinely have
// perfectly normal bikini/swimsuit photos, and an unqualified "nudity or
// sexually explicit content" instruction left the model free to treat
// ordinary beachwear as borderline and flag it out of caution. The line
// being enforced is nudity/sexual content, not modesty of dress.
const ANALYSIS_PROMPT = `Analyze this event photo. Also check whether it contains nudity or sexually explicit content that would be inappropriate for a general event recap shared with the host and their guests. Ordinary swimwear (bikinis, swim trunks, etc. at a pool party, beach, or vacation) is NOT nudity or sexual content and should never be flagged for that alone -- only flag actual nudity or sexually explicit content. Respond ONLY with JSON: {"technical_quality": 1-10, "emotional_strength": 1-10, "moment_type": "string", "notes": "short phrase", "flagged": boolean, "flag_reason": "short phrase or null"}`;
...
// an hour "most" of the time with no faster guarantee -- 6 hours gives a
// batch several real chances to finish (the scheduler now runs hourly, see
// .github/workflows/recap-scheduler.yml) before falling back, while leaving
// Luxe's 48-hour turnaround promise (the tightest of any tier) comfortable
// margin, since the rest of the pipeline (enhancement + video assembly)
// reliably finishes in minutes once analysis is done (see
