  ...
  return `
    ...
    <html lang="en">
      ...
      <body style="margin: 0; padding: 0; background-color: ${BG};">
        <div style="background-color: ${BG}; padding: 44px 16px; font-family: Arial, Helvetica, sans-serif;">
          <div style="max-width: 480px; margin: 0 auto;">
            <div style="background: ${CARD}; border: 1px solid ${BORDER}; border-radius: 22px; overflow: hidden; box-shadow: 0 14px 34px rgba(33,31,29,0.09);">
              <div style="background-color: ${TINT}; background-image: linear-gradient(160deg, ${TINT} 0%, ${BG} 100%); padding: 30px 28px 24px; text-align: center; border-bottom: 1px solid ${BORDER};">
                ...
                <p style="margin: 0; font-size: 11px; letter-spacing: 3px; text-transform: uppercase; color: ${SAGE}; font-weight: bold;">Recapped For You</p>
              </div>
              <div style="padding: 32px 28px 30px; color: ${HEADING};">
                ${innerHtml}
              </div>
            </div>
            <p style="text-align: center; font-size: 11px; color: ${FAINT}; margin: 22px 0 0;">
...
function sectionHeading(text, { color = HEADING, ruleColor = ACCENT } = {}) {
  return `
    <h2 style="font-family: Georgia, 'Times New Roman', serif; font-size: 26px; line-height: 1.3; margin: 0 0 12px; text-align: center; color: ${color};">${text}</h2>
...
function calloutBox(innerHtml) {
  return `<div style="background: ${BG}; border: 1px solid ${BORDER}; border-left: 3px solid ${ACCENT}; border-radius: 12px; padding: 20px 22px;">${innerHtml}</div>`;
}
...
export async function sendBookingConfirmation({ to, hostName, eventDate, eventType, guestCount, tier, style, amountPaid, roastEnabled, uploadUrl, uploadSlug, bookingId, deliveryFormat }) {
  ...
  return send({
    ...
    html: wrapEmail(`
        ${sectionHeading(`You're all set, ${hostName.split(" ")[0]}`, { color: BRIGHT_TEXT })}
        ...
        <div style="background-color: ${CORAL}; background-image: ${BRIGHT_GRADIENT}; border-radius: 18px; padding: 26px 22px;">
          ...
          <p style="margin: 20px 0 0; text-align: center;">
            <a href="${qrPageUrl}" style="display: inline-block; background: #FFFFFF; color: ${CORAL}; padding: 14px 26px; border-radius: 999px; text-decoration: none; font-weight: bold; font-size: 14.5px; box-shadow: 0 6px 18px rgba(0,0,0,0.15);">Share or print your QR code</a>
          </p>
        </div>
...
export async function sendConfirmBookingEmail({ to, hostName, eventDate, eventType, confirmUrl }) {
  return send({
    ...
    html: wrapEmail(`
        ...
        <p style="font-size: 12.5px; color: ${MUTED}; text-align: center; margin: 0 0 18px;">If you didn't request this, you can safely ignore this email — nothing goes live until this link is clicked.</p>
        ${calloutBox(`<p style="font-size: 13px; color: ${BODY}; line-height: 1.6; margin: 0;">Once confirmed, you'll get a second email right away with your guest QR code, your upload link, and everything else about your event.</p>`)}
    `),
  });
}

export async function sendCancellationConfirmation({ to, hostName, eventType, eventDate, refunded, amountRefunded }) {
  return send({
    to,
...
export async function sendRescheduleConfirmation({ to, hostName, oldDate, newDate }) {
  return send({
    ...
    html: wrapEmail(`
        ...
        <p style="font-size: 14.5px; line-height: 1.6; margin: -8px 0 18px; text-align: center;">Your event has been rescheduled from ${oldDate} to <strong style="color: ${HEADING};">${newDate}</strong> — no extra charge, and your guest upload link and QR code stay exactly the same.</p>
        ${calloutBox(`
...
export async function sendUploadReminder({ to, hostName, eventDate, uploadUrl, uploadSlug, bookingId, tier, uploadCount }) {
  ...
  // pass it still get a perfectly fine email, just without this one line.
  const countLine =
    typeof uploadCount === "number"
      ? `<p style="font-size: 14.5px; line-height: 1.6; margin: 0 0 12px; text-align: center;"><strong style="color: ${HEADING};">${uploadCount} photo${uploadCount === 1 ? "" : "s"}</strong> uploaded so far.</p>`
      : "";
...
export async function sendUploadCapReachedEmail({ to, hostName, eventType, tier, uploadSlug, bookingId }) {
  ...
  return send({
    ...
    html: wrapEmail(`
        ...
        <p style="font-size: 14.5px; line-height: 1.6; margin: -8px 0 18px; text-align: center;">${eventType} just reached the ${tierLabel} plan's limit of ${cap} photos. From here, any guest who tries to add another will see a message that the event is full — nothing breaks, but new uploads stop landing.</p>
        ${calloutBox(`
          ...
          <p style="font-size: 14px; color: ${BODY}; line-height: 1.6; margin: 0;"><strong style="color: ${HEADING};">Got everything you need?</strong> Close uploads now instead of waiting for your deadline, and we'll start putting your recap together right away.</p>
        `)}
        <p style="margin: 20px 0 0; text-align: center;">
          ${ctaButton(qrPageUrl, "Manage your photos")}
        </p>
        <p style="font-size: 13px; color: ${MUTED}; margin: 20px 0 0; text-align: center;">Need more than ${cap} photos for this event? Message us on WhatsApp: <a href="${WHATSAPP_URL}" style="color: ${MUTED};">${WHATSAPP_DISPLAY}</a>.</p>
    `),
  });
}
...
export async function sendDeliveryNotification({ to, hostName, galleryUrl, expiresDate, hasFullVideo, socialCutCount }) {
  ...
  return send({
    ...
    html: wrapEmail(`
        <div style="background-color: ${CORAL}; background-image: ${BRIGHT_GRADIENT}; border-radius: 18px; padding: 34px 26px; text-align: center;">
