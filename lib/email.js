...
// The only real contact channel (see app/page.jsx's Get In Touch section) --
// recappedforyou.com has no MX record configured, so a reply to FROM above
// would bounce. Same number/link used there.
  ...
  return resend.emails.send({
    ...
    html: wrapEmail(`
        ...
        ${calloutBox(`
          ...
          ${detailRow("⭐", "Star your favorites", `Once guests start uploading, star any must-include photos from your QR share page to guarantee they make the video${tier === "premium" || tier === "keepsake" ? " — social cuts get their own separate star picks too" : ""}, even if our automatic picks would've skipped them.`)}
...
export async function sendUploadReminder({ to, hostName, eventDate, uploadUrl, uploadSlug, tier }) {
  const qrPageUrl = `${process.env.APP_URL}/qr/${uploadSlug}`;
