...
const TIER_RETENTION_LABELS = {
  free: "7 days -- after that your photos and video are permanently removed",
  standard: "2 months",
  premium: "4 months",
  keepsake: "6 months",
};
...
export async function sendBookingConfirmation({ to, hostName, eventDate, eventType, guestCount, tier, style, amountPaid, roastEnabled, uploadUrl, uploadSlug }) {
  ...
  const summaryRow = (label, value) =>
    value
      ? `<tr><td style="padding: 7px 0; color: ${MUTED};">${label}</td><td style="padding: 7px 0; text-align: right; font-weight: 600;">${value}</td></tr>`
      : "";
  ...
  return resend.emails.send({
    ...
    html: wrapEmail(`
        <h2 style="font-family: Georgia, 'Times New Roman', serif; font-size: 25px; margin: 0 0 10px; text-align: center;">You're all set, ${hostName.split(" ")[0]}</h2>
        <p style="font-size: 14.5px; line-height: 1.6; margin: 0 0 22px; text-align: center;">Your event on ${eventDate} is booked. Share the QR code below with your guests so they can add their photos and videos.</p>
        <table style="width: 100%; border-collapse: collapse; margin: 0 0 24px; padding: 16px; background: ${TINT}; border-radius: 12px; font-size: 14px;">
        ...
        <p style="margin: 0 0 20px; text-align: center;">
          <img src="${qrImageUrl}" alt="Guest upload QR code" width="190" height="190" style="border-radius: 12px; border: 1px solid ${BORDER};" />
        </p>
        <p style="margin: 0 0 16px; text-align: center;">
          ${ctaButton(qrPageUrl, "Share or print your QR code")}
        </p>
        <p style="font-size: 12.5px; color: ${MUTED}; text-align: center; margin: 0 0 26px;">
          Or share the guest link directly: <a href="${uploadUrl}" style="color: ${MUTED};">${uploadUrl}</a>
        </p>

        <h3 style="font-family: Georgia, 'Times New Roman', serif; font-size: 16px; margin: 0 0 10px;">The details</h3>
        ${calloutBox(`
          <p style="font-size: 13.5px; color: ${BODY}; line-height: 1.6; margin: 0 0 10px;">Guests can upload for ${deadline} after your event — anything after that won't make the cut. Your QR share page (the button above) isn't just for sharing: you can add your own photos there too, or close uploads early once everyone's in.</p>
          <p style="font-size: 13.5px; color: ${BODY}; line-height: 1.6; margin: 0 0 10px;">Your recap will be delivered by email and stays accessible for ${retention} after delivery. Raw guest uploads are permanently deleted 30 days after final delivery.</p>
          ${roastEnabled ? `<p style="font-size: 13.5px; color: ${BODY}; line-height: 1.6; margin: 0 0 10px;">You've added the Roast Reel add-on — nothing is shared with your guests until you've reviewed and approved the full script by email.</p>` : ""}
          ${tier === "keepsake" ? `<p style="font-size: 13.5px; color: ${BODY}; line-height: 1.6; margin: 0 0 10px;">Luxe includes a one-time 2-day deadline extension if you need more time — also available from your QR share page.</p>` : ""}
          <p style="font-size: 13.5px; color: ${BODY}; line-height: 1.6; margin: 0 0 10px;">You can cancel any time before your event: more than 24 hours out gets a full refund, within 24 hours isn't eligible since guest uploads may already be underway.</p>
          <p style="font-size: 13.5px; color: ${BODY}; line-height: 1.6; margin: 0;">Your and your guests' photos and videos are used only to produce this recap — never sold, shared, or used for advertising.</p>
        `)}

        <p style="font-size: 12px; color: ${FAINT}; margin: 20px 0 0;">
          By completing your booking and payment, you agreed to these terms. <a href="${cancelUrl}" style="color: ${FAINT};">Cancel your booking</a>. Questions? Just reply to this email.
        </p>
    `, "🎬"),
  });
}
