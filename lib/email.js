        ...
        <div class="em-bg" bgcolor="${BG}" style="background-color: ${BG}; padding: 44px 16px; font-family: Arial, Helvetica, sans-serif;">
          <div style="max-width: 480px; margin: 0 auto;">
            <div class="em-card" style="background: ${CARD}; border: 1px solid ${BORDER}; border-radius: 22px; overflow: hidden; box-shadow: 0 14px 34px rgba(33,31,29,0.09);">
              <div class="em-tint" style="background-color: ${TINT}; background-image: linear-gradient(160deg, ${TINT} 0%, ${BG} 100%); padding: 30px 28px 0; text-align: center; border-bottom: 1px solid ${BORDER};">
                <!-- The site's own nav/favicon "R" monogram (see app/icon.svg), replacing the old per-email emoji badge -- see the removed-emoji comment above wrapEmail's declaration for why. -->
                <div style="width: 40px; height: 40px; line-height: 40px; margin: 0 auto 12px; border-radius: 12px; background-color: ${ACCENT}; font-family: Georgia, 'Times New Roman', serif; font-weight: bold; font-size: 19px; color: ${HEADING};">R</div>
                <p class="em-sage" style="margin: 0 0 20px; font-size: 11px; letter-spacing: 3px; text-transform: uppercase; color: ${SAGE}; font-weight: bold;">Recapped For You</p>
                <!-- A real carved crown-molding strip (see scripts/generate-invite-backgrounds.mjs-adjacent Roman-theme work; the same public-domain egg-and-dart profile the desktop site's flanking columns use), sitting at the boundary between the masthead and the card body the way a real entablature caps a column. Background-image, not an <img>, so it tiles edge to edge instead of one copy stretched and distorted -- degrades silently (no accent, no error) on the small share of clients that ignore CSS backgrounds, same tradeoff the gradient above it already makes. -->
                <div style="height: 14px; background-image: url('${process.env.APP_URL}/images/crown-molding.png'); background-repeat: repeat-x; background-size: auto 14px; background-position: top center;"></div>
              </div>
...
// would vanish against it. fontStyle lets sendBookingConfirmation mirror
// the invite card's own mood-based italic/upright split (NON_ITALIC_MOODS
// in lib/inviteMoods.js) with real CSS here instead of that card's vector-
// shear hack -- plain HTML/CSS text needs no synthetic italic.
function sectionHeading(text, { color = HEADING, ruleColor = ACCENT, emClass = "em-heading", fontStyle = "normal" } = {}) {
  return `
    <h2 class="${emClass}" style="font-family: Georgia, 'Times New Roman', serif; font-style: ${fontStyle}; font-size: 26px; line-height: 1.3; margin: 0 0 12px; text-align: center; color: ${color};">${text}</h2>
...
// ignores.
// `icon` is either a plain emoji (rendered in a flat tinted square, the
// original treatment) or a path into public/images/ starting with "/"
// (rendered as the actual hand-painted sticker used for that same concept
// elsewhere on the site -- see e.g. scripts/generate-faq-icons.mjs). Email
// clients don't resolve site-relative paths, so an image path is always
// expanded against APP_URL into a real absolute URL first.
function detailRow(icon, label, text) {
  const iconHtml = icon.startsWith("/")
    ? `<img src="${process.env.APP_URL}${icon}" width="32" height="32" alt="" style="border-radius: 9px; display: block;" />`
...
export async function sendBookingConfirmation({ to, hostName, eventDate, eventType, guestCount, tier, style, amountPaid, roastEnabled, uploadUrl, uploadSlug, bookingId, deliveryFormat }) {
  ...
  // Same mood/formality split as the guest invite card itself (lib/inviteCard.js) --
  // a wedding or birthday host reads a playful italic note, while corporate/religious
  // stay upright and professional. Real CSS font-style here; the card needs a vector-
  // shear hack instead because it's a rasterized image with no italic font loaded.
  const mood = getInviteMood(eventType);
  const italic = !NON_ITALIC_MOODS.has(mood.key) ? "italic" : "normal";
  ...
  // Surfaced upfront now instead of only after the host actually hits their
  // cap (sendUploadCapReachedEmail below) -- that email fires exactly once,
  // at the moment a guest upload lands exactly on the limit, which depends
  // on the send succeeding and the host actually opening it. A host who
  // already suspects a big turnout gets the option to act before running
  // into the wall at all. null for Premium/Luxe (already share the top cap)
  // AND for any tier string outside TIER_PRICES (e.g. a custom-quote
  // booking) -- nextUpgradeTier would happily return "premium" for an
  // unrecognized tier via getUploadLimit's 500-photo default, but there's no
  // real price difference to charge when the *starting* tier isn't a real
  // priced package, so the upgrade offer is suppressed rather than crashing
  // on TIER_PRICES[tier].amount.
  const upgradeTier = TIER_PRICES[tier] ? nextUpgradeTier(tier) : null;
  const cap = getUploadLimit(tier);
  // Spotlight/Luxe have no higher-cap tier to sell (see the comment above),
  // so they get the same WhatsApp custom-plan CTA sendUploadCapReachedEmail
  // falls back to -- offered here too, upfront, rather than only once the
  // host actually fills 2000 photos.
  const customPlanUrl = isSocialEligible
    ? `${WHATSAPP_URL}?text=${encodeURIComponent(
        `Hi! My ${eventType} is booked on ${TIER_LABELS[tier]} and I might need more than ${cap} photos of room -- can we talk about a custom plan?`
      )}`
    : null;
  ...
  const turnaround = tier === "keepsake" ? "usually ready within up to 48 hours (priority turnaround, Roast Reel included)" : "usually ready within a few days";
  ...
  return send({
    ...
    html: wrapEmail(`
        ${sectionHeading(`You're all set, ${hostName.split(" ")[0]}`, { color: BRIGHT_TEXT, emClass: "em-bright", fontStyle: italic })}
        <p style="font-size: 14.5px; font-style: ${italic}; line-height: 1.6; margin: -8px 0 22px; text-align: center; color: ${BRIGHT_TEXT};">Your event on ${eventDate} is booked. Share the QR code below with your guests so they can add their photos.</p>
        ...
        ${calloutBox(`
          ${detailRow("/images/faq-icons/whats-included.jpg", "What you'll get", `${whatYoullGet} Every photo your guests upload also lands in your photo gallery. Recaps are ${turnaround}.`)}
          ${detailRow("/images/how-it-works-icons/pitch-in.jpg", "Send a real invitation", `Your host page has a themed digital invite ready to go — event details and a QR code built right in, matched to your event's own style. WhatsApp or text delivers the whole picture in one tap, straight from your host page; email works too.`)}
          ${detailRow(
            "/images/faq-icons/guest-limit.jpg",
            "Guest uploads",
            `Open for ${deadline} after your event, with room for up to ${cap} photos on ${TIER_LABELS[tier]} — anything after that won't make the cut. Add your own photos or close uploads early from your QR share page above.${
              upgradeTier
                ? ` Expecting a bigger turnout? <a href="${process.env.APP_URL}/api/events/${uploadSlug}/upgrade?t=${hostToken}" style="color: ${BRIGHT_TEXT}; font-weight: 700;">Upgrade to ${TIER_LABELS[upgradeTier]}</a> anytime before you hit the limit — you'll only be charged the $${((TIER_PRICES[upgradeTier].amount - TIER_PRICES[tier].amount) / 100).toFixed(0)} difference.`
                : customPlanUrl
                ? ` Already expecting more than ${cap}? <a href="${customPlanUrl}" style="color: ${BRIGHT_TEXT}; font-weight: 700;">Message us on WhatsApp</a> and we'll work out a custom plan sized for it.`
                : ""
            }`
          )}
          ${detailRow("/images/faq-icons/pick-photos.jpg", "Star your favorites", `Once guests start uploading, star any must-include photos from your QR share page to guarantee they make the video${isSocialEligible && deliveryFormat !== "video_only" ? " — social cuts get their own separate star picks too" : ""}, even if our automatic picks would've skipped them.`)}
          ${detailRow("/images/faq-icons/retention.jpg", "Delivery & retention", `Your recap arrives by email and stays accessible for ${retention} after delivery, then it's permanently removed. Raw guest uploads are deleted 30 days after final delivery.`)}
          ${roastEnabled ? detailRow("/images/faq-icons/roast-reel.jpg", "Roast Reel", "Witty commentary layered over your recap video, plus a caption-free version alongside it. Social cuts aren't captioned — they play at the normal pace either way.") : ""}
          ${tier === "keepsake" ? detailRow("/images/faq-icons/turnaround.jpg", "Need more time?", "Luxe includes a one-time 2-day deadline extension — use it anytime before your uploads close, right from your QR share page.") : ""}
          ${detailRow("/images/faq-icons/payment.jpg", "Cancellations", "Cancelling more than 24 hours before your event gets you a full refund. Cancelling inside 24 hours isn't eligible, since guest uploads may already be underway.")}
          ${detailRow("/images/faq-icons/cancel-reschedule.jpg", "Need a new date?", "Reschedule for free up to 24 hours before your event, as long as your recap hasn't started processing yet — right from the link below. Your QR code and upload link stay the same.")}
          ${detailRow("/images/faq-icons/privacy.jpg", "Your privacy", "Photos are used only to produce this recap — never sold, shared, or used for advertising.")}
        `)}
...
// Emails the same digital invite image guests get shared via text/WhatsApp
// (see lib/inviteCard.js / app/api/invite/[slug]/route.js) directly to a
// guest's inbox, sent by app/api/events/[eventId]/send-invite/route.js.
// Exists because the "Share your event's digital invite" button
// (handleShareInvite in app/qr/[slug]/page.jsx) hands the image off to
// whatever app the host's OS share sheet picks -- on a messaging app that
// works (the picture arrives, the link rides along as text), but a mail
// app picked from the same share sheet often only carries the text/link
// through and drops the image file entirely, so an emailed invite showed
// up as a bare link with nothing to look at. Embedding the image directly
// in a real email guarantees the guest sees the actual card every time,
// regardless of which mail client they use.
export async function sendGuestInviteEmail({ to, eventName, dateTimeLine, venue, uploadUrl, inviteImageUrl }) {
  const detailsLine = [dateTimeLine, venue].filter(Boolean).join(" — ");
  // The digital invite is always 1080x1920 (9:16, see DIGITAL_CARD_W/H in
  // lib/inviteCard.js) -- fixed width/height attributes (not just a CSS
  // max-width) so mail clients that respect them reserve the right space
  // before the image itself loads, instead of a layout jump.
  const imageWidth = 340;
  const imageHeight = Math.round(imageWidth * (1920 / 1080));

  return send({
    to,
    subject: `You're invited to ${eventName}!`,
    html: wrapEmail(`
        ${sectionHeading(`You're invited to ${eventName}`)}
        ${detailsLine ? `<p style="font-size: 14px; color: ${MUTED}; text-align: center; margin: -8px 0 20px;">${detailsLine}</p>` : ""}
        <div style="text-align: center; margin: 0 0 22px;">
          <a href="${uploadUrl}">
            <img src="${inviteImageUrl}" alt="${eventName} invitation" width="${imageWidth}" height="${imageHeight}" style="max-width: 100%; height: auto; border-radius: 16px; display: inline-block; box-shadow: 0 10px 26px rgba(33,31,29,0.16);" />
          </a>
        </div>
        <p style="margin: 0 0 18px; text-align: center;">
          ${ctaButton(uploadUrl, "RSVP & Add Your Photos")}
        </p>
        <p style="font-size: 12.5px; color: ${MUTED}; text-align: center; margin: 0;">
          Or open this link directly: <a href="${uploadUrl}" style="color: ${MUTED};">${uploadUrl}</a>
        </p>
    `),
  });
}
...
export async function sendDeliveryNotification({ to, hostName, galleryUrl, expiresDate, hasFullVideo, socialCutCount }) {
  ...
  // Same "It lands in your inbox" sticker used on the homepage's How It
  // Works timeline and the guest page's own delivered state -- this is
  // that exact moment, from the host's side.
  return send({
    to,
    ...
    html: wrapEmail(`
        <div class="em-hero" style="background-color: ${CORAL}; background-image: ${BRIGHT_GRADIENT}; border-radius: 18px; padding: 34px 26px; text-align: center;">
          <img src="${process.env.APP_URL}/images/how-it-works-icons/delivered.jpg" width="56" height="56" alt="" style="border-radius: 16px; display: block; margin: 0 auto 16px;" />
