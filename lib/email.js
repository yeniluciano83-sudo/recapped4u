...
// Routes replies on a custom-inquiry email back to a per-inquiry address
// instead of the general hello@ inbox, so the Resend inbound webhook
// (app/api/webhooks/resend-inbound) can parse the inquiry id straight out
// of the recipient address and log the reply on that inquiry automatically
// -- see custom_inquiry_messages. Returns undefined (no replyTo override,
// same as before this existed) until RESEND_INBOUND_DOMAIN is configured,
// so nothing breaks while that Resend-side setup is still pending.
function inquiryReplyAddress(inquiryId) {
  if (!process.env.RESEND_INBOUND_DOMAIN || !inquiryId) return undefined;
  return `inquiry-${inquiryId}@${process.env.RESEND_INBOUND_DOMAIN}`;
}
  ...
  return resend.emails.send({
    ...
    html: wrapEmail(`
        ...
        ${calloutBox(`
          ...
          ${roastEnabled ? detailRow("🔥", "Roast Reel", "Witty commentary layered over your full recap video, plus a caption-free version of the same cut.") : ""}
...
export async function sendUploadReminder({ to, hostName, eventDate, uploadUrl, uploadSlug }) {
  ...
  return resend.emails.send({
    ...
    html: wrapEmail(`
        ...
        <p style="font-size: 14.5px; line-height: 1.6; margin: 0 0 12px; text-align: center;">Your event on ${eventDate} was yesterday. We'll automatically start putting your recap together in about 24 hours, using whatever photos have been uploaded by then.</p>
...
export async function sendCustomInquiry({ inquiryId, hostName, email, eventType, eventDate, guestCount, style, notes }) {
  ...
  return resend.emails.send({
    ...
    // Prefer the per-inquiry address once inbound is configured, so a
    // direct reply from the inbox (instead of using the dashboard) still
    // gets logged as an outbound message on the inquiry. Falls back to
    // replying straight to the host, same as before inbound existed.
    replyTo: inquiryReplyAddress(inquiryId) || email,
    ...
    html: `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; color: #211F1D;">
        ...
        <p style="font-size: 13px; color: #666;">Set a price for this in the <a href="${process.env.APP_URL}/dashboard?tab=custom">dashboard</a>, or reply directly to this email to talk scope with ${hostName.split(" ")[0]} first.</p>
      </div>
    `,
  });
}

export async function sendCustomInquiryReceived({ inquiryId, to, hostName, eventType }) {
  return resend.emails.send({
    from: FROM,
    to,
    replyTo: inquiryReplyAddress(inquiryId),
    subject: "We've got your custom package request",
    html: wrapEmail(`
        <h2 style="font-family: Georgia, 'Times New Roman', serif; font-size: 25px; margin: 0 0 10px; text-align: center;">Thanks, ${hostName.split(" ")[0]}</h2>
        <p style="font-size: 14.5px; line-height: 1.6; margin: 0 0 22px; text-align: center;">We've got your request for ${eventType}. We'll follow up by email with a quote once we've worked out the scope — usually within a couple of days.</p>
        <p style="font-size: 13px; color: ${MUTED}; text-align: center; margin: 0;">Want to add more detail in the meantime? Just reply to this email.</p>
    `, "📝"),
  });
}

export async function sendCustomQuote({ inquiryId, to, hostName, eventType, eventDate, tier, priceFormatted, message, quoteUrl }) {
  return resend.emails.send({
    from: FROM,
    to,
    replyTo: inquiryReplyAddress(inquiryId),
    subject: `Your custom package quote is ready — ${priceFormatted}`,
    html: wrapEmail(`
        <h2 style="font-family: Georgia, 'Times New Roman', serif; font-size: 25px; margin: 0 0 10px; text-align: center;">Here's what we've put together, ${hostName.split(" ")[0]}</h2>
        <p style="font-size: 14.5px; line-height: 1.6; margin: 0 0 18px; text-align: center;">A custom package for ${eventType} on ${eventDate}, built on our ${TIER_LABELS[tier] || tier} package.</p>
        ${calloutBox(`
          <p style="margin: 0 0 8px; font-size: 13px; color: ${MUTED}; text-transform: uppercase; letter-spacing: 0.06em;">Quoted price</p>
          <p style="margin: 0 0 ${message ? "14" : "0"}px; font-size: 28px; font-weight: bold; color: ${BRIGHT_TEXT};">${priceFormatted}</p>
          ${message ? `<p style="margin: 0; font-size: 14px; color: ${BODY}; line-height: 1.6;">${message}</p>` : ""}
        `)}
        <p style="margin: 20px 0 0; text-align: center;">
          ${ctaButton(quoteUrl, "Review & accept")}
        </p>
        <p style="font-size: 13px; color: ${MUTED}; text-align: center; margin: 16px 0 0;">Want to talk it through first? Just reply to this email — we're happy to adjust the quote.</p>
    `, "💌"),
  });
}

// A price-free reply -- for clarifying questions, or telling a host a
// request isn't doable, before there's anything to quote yet. Distinct
// from sendCustomQuote, which always centers a price.
export async function sendCustomInquiryReply({ inquiryId, to, hostName, eventType, message, declined }) {
  return resend.emails.send({
    from: FROM,
    to,
    replyTo: inquiryReplyAddress(inquiryId),
    subject: declined ? "About your custom package request" : `Re: your custom package request — ${eventType}`,
    html: wrapEmail(`
        <h2 style="font-family: Georgia, 'Times New Roman', serif; font-size: 22px; margin: 0 0 14px; text-align: center;">Hi ${hostName.split(" ")[0]},</h2>
        <p style="font-size: 14.5px; line-height: 1.7; margin: 0; white-space: pre-wrap;">${message}</p>
        <p style="font-size: 13px; color: ${MUTED}; text-align: center; margin: 22px 0 0;">Just reply to this email if you have questions or want to add anything.</p>
    `, declined ? "🙏" : "💬"),
  });
}

// Sent to the business by the inbound webhook whenever a host replies on
// a custom inquiry -- otherwise a new reply would just sit in the
// dashboard with nothing prompting anyone to go look at it.
export async function sendCustomInquiryReplyNotice({ hostName, eventType, preview, inquiryId }) {
  return resend.emails.send({
    from: FROM,
    to: "hello@recappedforyou.com",
    subject: `${hostName} replied — ${eventType}`,
    html: `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; color: #211F1D;">
        <h2>New reply on a custom inquiry</h2>
        <p style="font-size: 14px;"><strong>${hostName}</strong> replied about ${eventType}:</p>
        <blockquote style="margin: 0 0 16px; padding: 10px 14px; border-left: 3px solid #C97A3D; background: #FAF7F2; font-size: 14px; white-space: pre-wrap;">${preview}</blockquote>
        <p style="font-size: 13px; color: #666;"><a href="${process.env.APP_URL}/dashboard?tab=custom&inquiry=${inquiryId}">View in dashboard</a></p>
      </div>
    `,
  });
}
