...
describe("email templates", () => {
  ...
  describe("sendUploadReminder", () => {
    ...
    it("mentions the current upload count when given one, including the zero case", async () => {
      ...
      expect(resendMocks.send.mock.calls[0][0].html).toContain("12 photos");

      await sendUploadReminder({ to: "jordan@example.com", hostName: "Jordan", eventDate: "Dec 25", uploadUrl: "https://test.example/event/slug-1", uploadSlug: "slug-1", tier: "standard", uploadCount: 1 });
      expect(resendMocks.send.mock.calls[1][0].html).toContain("1 photo</strong>"); // singular, not "1 photos"
      ...
      expect(resendMocks.send.mock.calls[2][0].html).toContain("0 photos");
    });

    it("omits the upload-count line entirely when uploadCount isn't given", async () => {
