...
describe("email templates", () => {
  ...
  describe("sendBookingConfirmation", () => {
    ...
    it("offers an upgrade link with the correct price difference for a tier that isn't already at the top cap", async () => {
      await sendBookingConfirmation({ ...BASE_BOOKING, tier: "standard" }); // Highlight -> Spotlight, $75-$35=$40
      const html = resendMocks.send.mock.calls[0][0].html;
      expect(html).toContain("Upgrade to Spotlight");
      expect(html).toContain("$40 difference");
      expect(html).toContain("/api/events/slug-1/upgrade?t=");
    });

    it("swaps the upgrade offer for a WhatsApp custom-plan CTA on Premium/Luxe, which already share the top upload cap", async () => {
      await sendBookingConfirmation({ ...BASE_BOOKING, tier: "premium" });
      const premiumHtml = resendMocks.send.mock.calls[0][0].html;
      expect(premiumHtml).not.toContain("Upgrade to");
      expect(premiumHtml).toContain("work out a custom plan sized for it");
      expect(premiumHtml).toContain("https://wa.me/16465129151?text=");

      await sendBookingConfirmation({ ...BASE_BOOKING, tier: "keepsake" });
      const keepsakeHtml = resendMocks.send.mock.calls[1][0].html;
      expect(keepsakeHtml).not.toContain("Upgrade to");
      expect(keepsakeHtml).toContain("work out a custom plan sized for it");
    });

    it("omits the proactive WhatsApp custom-plan CTA (distinct from the footer's general WhatsApp line) on tiers below Premium/Luxe", async () => {
      await sendBookingConfirmation({ ...BASE_BOOKING, tier: "standard" });
      expect(resendMocks.send.mock.calls[0][0].html).not.toContain("work out a custom plan sized for it");
    });

    it("falls back to the raw tier/style id when it's not in the label maps, without crashing on the upgrade-price lookup", async () => {
      ...
      // A tier string outside TIER_PRICES (a custom-quote booking) has no
      // real price to diff against, so the upgrade offer is suppressed
      // rather than throwing on TIER_PRICES[tier].amount.
    ...
    // summaryRow used to check bare truthiness, which treats 0 the same as a
    // missing value -- a genuinely-entered guest count of exactly 0 (a
    // solo/self-only event) silently vanished from the summary card instead
    // of showing "0".
    it("still shows the Guests row when guestCount is exactly 0", async () => {
      await sendBookingConfirmation({ ...BASE_BOOKING, guestCount: 0 });
      const html = resendMocks.send.mock.calls[0][0].html;
      expect(html).toContain("Guests");
      expect(html).toMatch(/Guests<\/td><td[^>]*>0</);
    });

    // poll-and-recap.js's purgeExpiredGalleries deletes every tier's files
    // identically once gallery_purge_at passes -- Highlight/Spotlight/Luxe
    // get no special treatment over Free. Saying this only for Free
    // (the old copy) told paying customers nothing about their own content
    // being permanently deleted too.
    it("mentions permanent removal after the retention window for every tier, not just Free", async () => {
      for (const tier of ["free", "standard", "premium", "keepsake"]) {
        await sendBookingConfirmation({ ...BASE_BOOKING, tier });
      }
      for (const call of resendMocks.send.mock.calls) {
        expect(call[0].html).toContain("permanently removed");
      }
    });
  ...
  describe("sendCustomQuoteEmail", () => {
    it("includes the price, the checkout link, and a plan name derived from the tier when no label is given", async () => {
      await sendCustomQuoteEmail({ to: "jordan@example.com", hostName: "Jordan Smith", eventType: "Corporate Event", eventDate: "December 25, 2026", tier: "premium", amount: "$500.00", checkoutUrl: "https://checkout.stripe.com/quote1" });
      const call = resendMocks.send.mock.calls[0][0];
      expect(call.to).toBe("jordan@example.com");
      expect(call.html).toContain("$500.00");
      expect(call.html).toContain("https://checkout.stripe.com/quote1");
      expect(call.html).toContain("Spotlight-based custom plan");
    });

    it("uses the given label as the plan name instead of the tier-derived default", async () => {
      await sendCustomQuoteEmail({ to: "jordan@example.com", hostName: "Jordan", eventType: "Wedding", eventDate: "Dec 25", tier: "keepsake", amount: "$950.00", checkoutUrl: "https://checkout.stripe.com/quote2", label: "2-Day Wedding Coverage" });
      const html = resendMocks.send.mock.calls[0][0].html;
      expect(html).toContain("2-Day Wedding Coverage");
      expect(html).not.toContain("Luxe-based custom plan");
    });

    it("includes the description only when one is provided", async () => {
      await sendCustomQuoteEmail({ to: "jordan@example.com", hostName: "Jordan", eventType: "Wedding", eventDate: "Dec 25", tier: "standard", amount: "$99.00", checkoutUrl: "https://checkout.stripe.com/quote3", description: "Includes both ceremony and reception" });
      expect(resendMocks.send.mock.calls[0][0].html).toContain("Includes both ceremony and reception");

      await sendCustomQuoteEmail({ to: "jordan@example.com", hostName: "Jordan", eventType: "Wedding", eventDate: "Dec 25", tier: "standard", amount: "$99.00", checkoutUrl: "https://checkout.stripe.com/quote4" });
      expect(resendMocks.send.mock.calls[1][0].html).not.toContain("Details");
    });
  });
  ...
  describe("sendCancellationConfirmation", () => {
    ...
    it("explains the 24h policy when a paid, timing-ineligible booking isn't refunded", async () => {
      await sendCancellationConfirmation({ to: "jordan@example.com", hostName: "Jordan", eventDate: "Dec 25", tier: "standard", refunded: false, refundEligible: false, amountRefunded: null });
      const html = resendMocks.send.mock.calls[0][0].html;
    ...
    // The bug this replaced: !refunded was always blamed on 24h timing, even
    // for a Free booking cancelled weeks out -- there was simply never a
    // charge to refund, which is a different, true reason.
    it("says there was nothing to refund for a Free-tier cancellation, not that it was too late", async () => {
      await sendCancellationConfirmation({ to: "jordan@example.com", hostName: "Jordan", eventDate: "Dec 25", tier: "free", refunded: false, refundEligible: true, amountRefunded: null });
      const html = resendMocks.send.mock.calls[0][0].html;
      expect(html).toContain("nothing to refund");
      expect(html).not.toContain("within 24 hours");
    });

    // Paid, timing was fine, but refunded is still false -- a real Stripe/data
    // failure, not a policy outcome. Telling the host it was "too late" here
    // would be just as wrong as the Free-tier case.
    it("flags a real refund failure distinctly when a paid, timing-eligible booking still isn't refunded", async () => {
      await sendCancellationConfirmation({ to: "jordan@example.com", hostName: "Jordan", eventDate: "Dec 25", tier: "standard", refunded: false, refundEligible: true, amountRefunded: null });
      const html = resendMocks.send.mock.calls[0][0].html;
      expect(html).toContain("couldn't be processed automatically");
      expect(html).not.toContain("within 24 hours");
      expect(html).not.toContain("nothing to refund");
    });
    ...
    it("names which event was cancelled, not just the date", async () => {
      await sendCancellationConfirmation({ to: "jordan@example.com", hostName: "Jordan", eventType: "Wedding", eventDate: "Dec 25", tier: "standard", refunded: false, refundEligible: false, amountRefunded: null });
      const html = resendMocks.send.mock.calls[0][0].html;
  ...
  describe("sendBookingUpdateConfirmation", () => {
    it("states the new tier and its real upload cap", async () => {
      await sendBookingUpdateConfirmation({ to: "jordan@example.com", hostName: "Jordan", tier: "premium", customPriceCents: null });
      const call = resendMocks.send.mock.calls[0][0];
      expect(call.html).toContain("Spotlight");
      expect(call.html).toContain("2000"); // getUploadLimit("premium")
    });

    it("includes the custom price only when one is set", async () => {
      await sendBookingUpdateConfirmation({ to: "jordan@example.com", hostName: "Jordan", tier: "keepsake", customPriceCents: 15000 });
      expect(resendMocks.send.mock.calls[0][0].html).toContain("$150.00");

      await sendBookingUpdateConfirmation({ to: "jordan@example.com", hostName: "Jordan", tier: "keepsake", customPriceCents: null });
      expect(resendMocks.send.mock.calls[1][0].html).not.toContain("custom price");
    });

    it("carries no payment framing -- distinct from the self-service upgrade confirmation", async () => {
      await sendBookingUpdateConfirmation({ to: "jordan@example.com", hostName: "Jordan", tier: "premium", customPriceCents: null });
      const html = resendMocks.send.mock.calls[0][0].html.toLowerCase();
      expect(html).not.toContain("upgraded");
      expect(html).not.toContain("charge");
    });

    it("reassures the guest link and QR code didn't change", async () => {
      await sendBookingUpdateConfirmation({ to: "jordan@example.com", hostName: "Jordan", tier: "standard", customPriceCents: null });
      expect(resendMocks.send.mock.calls[0][0].html.toLowerCase()).toContain("stays exactly the same");
    });
  });
