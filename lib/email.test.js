...
describe("email templates", () => {
  ...
  describe("sendBookingConfirmation", () => {
    ...
    it("omits the upgrade offer for Premium/Luxe, which already share the top upload cap", async () => {
      await sendBookingConfirmation({ ...BASE_BOOKING, tier: "premium" });
      expect(resendMocks.send.mock.calls[0][0].html).not.toContain("Upgrade to");

      await sendBookingConfirmation({ ...BASE_BOOKING, tier: "keepsake" });
      expect(resendMocks.send.mock.calls[1][0].html).not.toContain("Upgrade to");
    });
