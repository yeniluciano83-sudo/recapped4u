...
function buildDigitalOverlay({ hostName, eventType, dateTimeLine, venue, uploadUrl, mood }) {
  ...
  const footerScrim = `<rect x="${footerScrimX}" y="${footerY - 20}" width="${footerScrimWidth}" height="${footerY + URL_OFFSET + 16 - (footerY - 20)}" rx="24" fill="${mood.card}" opacity="0.4" />`;
...
function buildPrintOverlay({ hostName, eventType, dateTimeLine, venue, uploadUrl, mood }) {
  ...
  const rightScrim = `<rect x="${rightScrimX}" y="${qrFrameY - scrimPad}" width="${rightScrimWidth}" height="${footerY + URL_OFFSET + 16 - (qrFrameY - scrimPad)}" rx="24" fill="${mood.card}" opacity="0.4" />`;
