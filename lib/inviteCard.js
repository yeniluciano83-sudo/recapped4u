...
// Every mood's background is a 3D render now (see scripts/generate-invite
// -backgrounds.mjs), but only these two keep upright type -- corporate
// and religious ceremony invitations read as formal occasions, and an
// italic slant reads as playful/personal, which is exactly the note the
// other five moods are leaning into on purpose. This split is about type
// style only; it has no bearing on which moods get which flavor of 3D art.
const NON_ITALIC_MOODS = new Set(["professional", "reverent"]);
...
function buildDigitalOverlay({ hostName, eventType, dateTimeLine, venue, uploadUrl, mood }) {
  ...
  const rsvpPillsSvg = rsvpPills
    ...
    ${textPath(label, x + w / 2, textY, RSVP_PILL_FONT_SIZE, mood.text, { align: "center", italic })}`;
    })
    .join("\n");
...
<svg width="${CARD_W}" height="${CARD_H}" viewBox="0 0 ${CARD_W} ${CARD_H}" xmlns="http://www.w3.org/2000/svg">
  ...
  ${textPath("WILL YOU BE THERE?", CARD_W / 2, rsvpLabelY, RSVP_LABEL_SIZE, mood.muted, { align: "center", italic })}
  ${rsvpPillsSvg}
  ...
  ${textPath(uploadUrl, CARD_W / 2, footerY + URL_OFFSET, URL_SIZE, mood.muted, { align: "center", opacity: 0.75, italic })}
</svg>`;
...
<svg width="${CARD_W}" height="${CARD_H}" viewBox="0 0 ${CARD_W} ${CARD_H}" xmlns="http://www.w3.org/2000/svg">
  ...
  ${textPath(uploadUrl, rightColCenterX, footerY + URL_OFFSET, URL_SIZE, mood.muted, { align: "center", opacity: 0.75, italic })}
</svg>`;
...
export async function buildInviteCard({ hostName, eventType, eventDate, eventTime, venue, uploadUrl, dateLabel, timeLabel, showRsvp = true }) {
  ...
  const [backgroundBuffer, overlayBuffer, qrBuffer] = await Promise.all([
    ...
    sharp(backgroundPath).resize(CARD_W, CARD_H, { fit: "cover" }).sharpen({ sigma: 1.1 }).png().toBuffer()
