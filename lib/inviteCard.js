...
export async function buildInviteCard({ hostName, eventType, eventDate, eventTime, venue, uploadUrl, dateLabel, timeLabel }) {
  ...
  const scrimPad = 26;
  const mainScrim = `<rect x="${contentX - scrimPad}" y="${eyebrowY - 50}" width="${contentRight - contentX + scrimPad * 2}" height="${rsvpPillY + RSVP_PILL_HEIGHT + 14 - (eyebrowY - 50)}" rx="24" fill="${mood.card}" opacity="0.6" />`;
...
<svg width="${CARD_W}" height="${CARD_H}" viewBox="0 0 ${CARD_W} ${CARD_H}" xmlns="http://www.w3.org/2000/svg">
  ...
  ${footerScrim}
  ${textPath("Scan or tap to RSVP & add your photos", CARD_W / 2, footerY, FOOTER1_SIZE, mood.muted, { align: "center" })}
