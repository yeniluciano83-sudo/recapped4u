...
function buildPrintOverlay({ hostName, eventType, dateTimeLine, venue, uploadUrl, mood }) {
  ...
  const rightScrim = `<rect x="${qrFrameX - scrimPad}" y="${qrFrameY - scrimPad}" width="${qrFrameSize + scrimPad * 2}" height="${footerY + URL_OFFSET + 16 - (qrFrameY - scrimPad)}" rx="24" fill="${mood.card}" opacity="0.6" />`;
