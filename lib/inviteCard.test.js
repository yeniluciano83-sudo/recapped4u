...
describe("buildInviteCard", () => {
  ...
  it("renders a genuinely different image when showRsvp is false, for the printed card", async () => {
    // The printed card (app/qr/[slug]/page.jsx's print-card) gets taped up
    // at the event itself and uses its own landscape layout/copy ("Scan to
    // add your photos" instead of the digital invite's pre-event "Scan or
    // tap to RSVP...") -- confirms the flag actually changes the rendered
    // image rather than being silently ignored. Neither variant draws real
    // RSVP controls any more (a static image/printed card can't respond to
    // a tap either way) -- that only ever lived on the real guest page.
