...
describe("buildInviteCard", () => {
  ...
  it("picks a different background color for a different event-type mood", async () => {
    ...
    const romanticCorner = await sharp(romantic).extract({ left: 0, top: 0, width: 1, height: 1 }).raw().toBuffer();
    const professionalCorner = await sharp(professional).extract({ left: 0, top: 0, width: 1, height: 1 }).raw().toBuffer();

    expect(Buffer.compare(romanticCorner, professionalCorner)).not.toBe(0);
  });
