...
describe("getInviteMood", () => {
  ...
  it("maps Party to its own party mood -- it's a real selectable event type, not a fallback bucket", () => {
    expect(getInviteMood("Party").key).toBe("party");
  });

  it("splits Birthday from Party into its own milestone mood", () => {
    expect(getInviteMood("Birthday").key).toBe("birthday");
    expect(getInviteMood("Party").key).not.toBe(getInviteMood("Birthday").key);
  });

  it("falls back to generic for Other", () => {
