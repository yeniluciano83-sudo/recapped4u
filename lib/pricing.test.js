...
import { TIER_PRICES, SOCIAL_CUT_ELIGIBLE_TIERS, ROAST_FULL_LEVELS_TIERS, roastAddonPriceCents, defaultGalleryTemplate } from "./pricing";
...
describe("defaultGalleryTemplate", () => {
  it("defaults social-cuts-only to polaroid", () => {
    expect(defaultGalleryTemplate("social_cuts")).toBe("polaroid");
  });

  it("defaults every other format to grid, matching the DB's own default", () => {
    expect(defaultGalleryTemplate("recap")).toBe("grid");
    expect(defaultGalleryTemplate("video_only")).toBe("grid");
    expect(defaultGalleryTemplate(undefined)).toBe("grid"); // tiers below Spotlight/Luxe never set a format at all
  });
});
