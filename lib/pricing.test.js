...
import { TIER_PRICES, SOCIAL_CUT_ELIGIBLE_TIERS, ROAST_FULL_LEVELS_TIERS, roastAddonPriceCents, isRoastLevelSelectable, defaultGalleryTemplate, nextUpgradeTier } from "./pricing";
...
describe("nextUpgradeTier", () => {
  it("recommends Highlight (500) as the cheapest way off Free's 20-photo cap", () => {
    expect(nextUpgradeTier("free")).toBe("standard");
  });

  it("recommends Spotlight (2000), not Luxe, as the cheaper of the two tiers that raise Highlight's 500-photo cap", () => {
    expect(nextUpgradeTier("standard")).toBe("premium");
  });

  // Spotlight and Luxe share the same 2000-photo cap -- nothing raises it
  // any further, so there's nothing to recommend.
  it("has no upgrade to offer from Spotlight or Luxe -- they already share the top cap", () => {
    expect(nextUpgradeTier("premium")).toBeNull();
    expect(nextUpgradeTier("keepsake")).toBeNull();
  });
});
