 ...
 * @param {{ eventType: string, roastLevel: "light" | "lukewarm" | "hot", recentLines?: string[], hostContext?: string }} context
 ...
 *   hostContext - free-text facts from the host (the booking's notes, or an
 *   explicit override on a re-render): names, who's who, relationships,
 *   inside references. Treated as ground truth; without it the model is
 *   told not to guess at how people are related.
 * @returns {Promise<Array<{ photo_index: number, line: string }>>}
 */
async function generateRoastScript(photos, { eventType, roastLevel, recentLines, hostContext }) {
...
async function generateRoastScriptChunk(photos, { eventType, roastLevel, recentLines, hostContext }) {
  ...
  // Optional facts from the host (the booking's notes field, or an explicit
  // override on a re-render) -- names, who's who, relationships, inside
  // references. The model treats these as ground truth; without them it must
  // not guess at relationships at all (see the rule in the prompt).
  const hostContextBlock = hostContext && hostContext.trim()
    ? `\n\nContext from the host -- these are facts. Use them and never contradict them:\n${hostContext.trim()}\n`
    : "";
...
Don't state or imply how the people in a photo are related to each other -- friends, a couple, siblings, parent and child, coworkers. You can't know that from a photo, and guessing wrong (calling sisters "friends", a relative a "date") lands badly and the joke never recovers. Build the joke on what someone is doing or how the scene feels, not on a relationship you've assumed. Only name a relationship, or a person, if the host context below states it.${hostContextBlock}
You'll see ${photos.length} photos below, labeled Photo 0 through Photo ${photos.length - 1}, in the order they'll appear in the video. Write ONE line per photo -- punchy, spoken-aloud length, this gets read as an overlay/voiceover, not a paragraph. Keep it under 80 characters (roughly a short sentence) so it wraps to no more than two lines on screen -- a longer line wraps onto three or more and crowds the photo it's captioning. Vary your joke structures and angles across the set -- don't reuse the same setup twice.
${recentLinesBlock}
