...
// For a signal worth knowing about that isn't an actual exception -- e.g. a
// guest's upload batch ending with photos that never made it in despite
// retrying. captureError/captureException expects a real Error and reads
// oddly for something that isn't one.
export function captureMessage(message, context = {}) {
  if (!process.env.SENTRY_DSN) return;
  Sentry.captureMessage(message, { level: "warning", tags: context.tags, extra: context.extra });
}
