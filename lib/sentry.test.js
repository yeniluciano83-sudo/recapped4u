...
import { captureError, captureMessage, flushSentry } from "./sentry";
...
describe("lib/sentry (unconfigured -- no SENTRY_DSN)", () => {
  ...
  it("captureMessage does not throw when given tags/extra context", () => {
    expect(() => captureMessage("test message", { tags: { route: "x" }, extra: { bookingId: "b1" } })).not.toThrow();
  });
});
