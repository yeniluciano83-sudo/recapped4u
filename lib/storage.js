...
// Full object body as a Buffer, for the one case that actually needs the
// whole file in memory at once: compositing a polaroid frame onto a single
// photo (lib/photo-frame.js) can't work off a stream, since the border size
// depends on the photo's own dimensions. Called one photo at a time in both
// callers (never Promise.all'd across a whole gallery), so peak memory stays
// bounded to one photo's size regardless of how many thousand are in it.
