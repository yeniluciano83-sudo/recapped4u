  ...
  return new Promise((resolve, reject) => {
    ...
    const photoFilters = imagePaths.map((_, i) => {
      ...
      if (photoBackground === "black") {
        ...
        // looks the same at 1080p previews and 4K finals.
        const cardMaxW = Math.round(outputWidth * 0.62);
        const cardMaxH = Math.round(outputHeight * 0.72);
        const border = Math.round(outputWidth * 0.016);
        const borderBottom = Math.round(outputHeight * 0.085);
        chain =
          `[${i}:v]scale=${cardMaxW}:${cardMaxH}:force_original_aspect_ratio=decrease,` +
          `pad=iw+2*${border}:ih+${border}+${borderBottom}:${border}:${border}:color=white,` +
          `pad=${outputWidth}:${outputHeight}:(ow-iw)/2:(oh-ih)/2:color=black`;
      } else {
        chain =
