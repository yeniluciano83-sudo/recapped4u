...
const SECONDS_PER_SLOT = 4; // shared duration for both photo and video-clip slots, so crossfade timing stays uniform
...
// slots), each padded/trimmed to the same SECONDS_PER_SLOT as photos so
// the crossfade offsets below stay correct across the whole sequence.
  ...
  return new Promise((resolve, reject) => {
    ...
    const photoFilters = imagePaths.map((_, i) => {
      let chain =
        ...
        `zoompan=z=1:d=${Math.round(SECONDS_PER_SLOT * 25)}:s=${OUTPUT_WIDTH}x${OUTPUT_HEIGHT}:fps=25,` +
        `trim=duration=${SECONDS_PER_SLOT},setpts=PTS-STARTPTS`;
    ...
    const clipFilters = clipPaths.map((_, j) => {
      ...
      // tpad always adds up to SECONDS_PER_SLOT of cloned-last-frame padding
      // regardless of the clip's actual length, so the stream is guaranteed
      // to be at least SECONDS_PER_SLOT long by the time trim runs --
      // correct whether the source clip is shorter or longer than a slot,
      // with no need to probe its duration first.
      return (
        `[${i}:v]scale=${OUTPUT_WIDTH}:${OUTPUT_HEIGHT}:force_original_aspect_ratio=decrease,` +
        `pad=${OUTPUT_WIDTH}:${OUTPUT_HEIGHT}:(ow-iw)/2:(oh-ih)/2:color=black,fps=25,` +
        `tpad=stop_mode=clone:stop_duration=${SECONDS_PER_SLOT},` +
        `trim=duration=${SECONDS_PER_SLOT},setpts=PTS-STARTPTS[v${i}]`
      );
    });
    ...
    for (let i = 1; i < slotCount; i++) {
      const outLabel = `vx${i}`;
      const offset = i * SECONDS_PER_SLOT - i * TRANSITION_SECONDS;
