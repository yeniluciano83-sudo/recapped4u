...
function renderSlotRange(imagePaths, clipPaths, roastLines, overlayLines, outputPath, opts) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // Polaroid card geometry, computed once here (rather than per-photo
    // inside the map below) because the roast caption box below also needs
    // it, to keep the caption off the card instead of overlapping its
    // white bottom border. See the photoBackground chain-building comment
    // in the map for what each of these means; cardMaxW isn't needed here
    // since only the card's HEIGHT matters for a bottom margin.
    // 0.72 originally, deliberately floating the card with generous room
    // around it -- reported live as the black backdrop "taking up
    // significant space" once real photos were on screen. A portrait photo
    // still pillarboxes hard in a landscape frame no matter what (the
    // unavoidable, already-settled tradeoff against cropping), but that's
    // no reason to ALSO add a big decorative margin on top of it -- 0.86
    // shrinks the added margin to a thin frame instead of a floating card.
    const polaroidCardMaxH = Math.round(outputHeight * 0.86);
    const polaroidBorder = Math.round(outputWidth * 0.016);
    const polaroidBorderBottom = Math.round(outputHeight * 0.085);
    // The card is centered vertically, so however tall the actual card
    // ends up (bounded above by polaroidCardMaxH -- a photo's own aspect
    // ratio only ever makes it shorter, never taller), at least this much
    // black backdrop always separates its bottom edge from the frame's own
    // bottom edge. A caption placed at or below this margin can never
    // overlap the card, for any photo.
    const polaroidCaptionBottomMargin = Math.round((outputHeight - (polaroidCardMaxH + polaroidBorder + polaroidBorderBottom)) / 2);
    ...
    const photoFilters = imagePaths.map((_, i) => {
      ...
      //                 caption strip), and the finished card is centered
      //                 on a black backdrop. Same cost profile as "black"
      //                 (no split/gblur), just two extra cheap pads.
      let chain;
      if (photoBackground === "black") {
        ...
        // Card fits within a box smaller than the full frame so the black
        // backdrop always shows around it; the bottom border is much
        // thicker than the other three sides for the classic polaroid
        // shape. Values are proportional to the output resolution so this
        // looks the same at 1080p previews and 4K finals. Height-related
        // values are shared with polaroidCaptionBottomMargin above (the
        // roast caption box needs to know how tall the card can get too).
        // 0.62 originally -- widened alongside polaroidCardMaxH above, same
        // reasoning (a thin frame around the card, not a floating one). A
        // landscape photo is the only one this ceiling actually binds for;
        // a portrait photo is always height-bound regardless.
        const cardMaxW = Math.round(outputWidth * 0.92);
        chain =
          `[${i}:v]scale=${cardMaxW}:${polaroidCardMaxH}:force_original_aspect_ratio=decrease,` +
          `pad=iw+2*${polaroidBorder}:ih+${polaroidBorder}+${polaroidBorderBottom}:${polaroidBorder}:${polaroidBorder}:color=white,` +
          `pad=${outputWidth}:${outputHeight}:(ow-iw)/2:(oh-ih)/2:color=black`;
      } else {
        chain =
      ...
      if (roastLines && roastLines[i]) {
        ...
        // original small fixed offset in "blur"/"black" mode, where the
        // caption box sitting over the bottom of the photo itself is the
        // whole design (same as every Instagram-style caption bar).
        //
        // "polaroid" is different: below the card is black backdrop, not
        // more photo, and that backdrop (polaroidCaptionBottomMargin) isn't
        // tall enough to fit a multi-line caption box on most cards -- the
        // fixed offset above just left the box straddling the card's white
        // border, part on crisp white and part on black (confirmed live).
        // A tall caption box has nowhere clean to go but back onto the
        // photo, same as every other mode -- so instead of anchoring the
        // box a fixed distance off the frame bottom (which lets its height
        // grow upward into the border by however tall that particular
        // line wraps), anchor its BOTTOM edge to the photo/border boundary
        // itself (the worst case, i.e. tallest possible card -- true for
        // most photos, since portrait ones typically hit that height cap;
        // a wider photo's shorter card just leaves it floating a bit above
        // its own border, never touching it, which reads fine). The box
        // then always grows upward onto the photo, never down across the
        // border.
        const roastBottomMargin =
          isVertical ? Math.round(outputHeight * 0.18)
          : photoBackground === "polaroid" ? polaroidCaptionBottomMargin + polaroidBorderBottom + roastBoxBorderW
          : Math.round(70 * textScale);
        chain +=
