...
// getAudioDurationSeconds below is the only thing here that needs ffprobe
// (everything else derives duration from known slot counts, never by
// probing a real file) -- same FFPROBE_PATH escape hatch as FFMPEG_PATH
// above, for the same reason (a CI/system binary swap-in).
...
// Handwriting font for the roast line written into a polaroid card's bottom
// strip -- reads as "captioned by hand" rather than a UI label. Kalam
// (SIL OFL 1.1, see fonts/Kalam-OFL.txt) -- a legible, slightly casual
// hand. Falls back to Oswald if the file is ever missing so the render
// never breaks over it.
const POLAROID_CAPTION_FONT_PATH = (() => {
  const hand = path.join(__dirname, "fonts", "Kalam-Bold.ttf");
  return fs.existsSync(hand) ? hand : FONT_PATH;
})();

// Ken Burns zoom SPEED, in zoom-units per real second -- not a fixed end
// zoom. A roast-captioned photo (ROAST_SLOT_SECONDS, 10s) sits on screen
// 2.5x longer than a title card or a non-roast photo (SECONDS_PER_SLOT,
// 4s); the old code instead picked a fixed END zoom (e.g. 1.1) and spread
// it evenly over whatever duration that slot happened to get, so the same
// nominal "zoom" moved noticeably slower on a long roast slot than on a
// short card -- confirmed from a live delivered video where the intro/
// outro cards visibly pushed in faster than the roast photos around them.
// A constant rate makes every slot's zoom feel like the same continuous
// motion regardless of duration; a longer slot simply ends up more zoomed
// in, which reads as normal rather than "the motion changed." Calibrated
// so a 4s slot still reaches the old 1.1 end zoom, unchanged from before.
const KEN_BURNS_RATE_PER_SECOND = 0.1 / SECONDS_PER_SLOT;
// A snappier rate reserved for a single deliberate "hook" slot (see
// heroZoomIndex below) -- calibrated so a 4s slot reaches the old 1.18.
const HERO_ZOOM_RATE_PER_SECOND = 0.18 / SECONDS_PER_SLOT;
// Safety ceiling on total zoom regardless of how long a slot runs (a very
// long slot -- e.g. a tier with a fixed target duration and few photos --
// would otherwise keep pushing in for its whole duration). Comfortably
// above anything today's slot durations reach in practice.
const KEN_BURNS_MAX_ZOOM = 1.4;
...
function renderSlotRange(imagePaths, clipPaths, roastLines, overlayLines, outputPath, opts) {
  ...
  const {
    ...
    // "black" uses plain letterbox/pillarbox bars instead. "polaroid" insets
    // the photo on a white card (thicker border at the bottom, like a real
    // polaroid) centered on a black backdrop. "black"/"polaroid" both skip
    // the split + second full-res scale + the gblur, which is by far the
    // most expensive filter in this chain.
  ...
  return new Promise((resolve, reject) => {
    ...
    // Polaroid card geometry, computed once here (rather than per-photo in
    // the map below). See the photoBackground chain-building comment in the
    // map for what each does. cardMaxH sits just under the frame height so
    // the blurred backdrop shows mostly on the sides; the bottom border is
    // a real caption strip -- tall enough to hold the roast line, written
    // INTO the strip like a real polaroid rather than over the photo.
    const polaroidCardMaxH = Math.round(outputHeight * 0.72);
    const polaroidBorder = Math.round(outputWidth * 0.014);
    const polaroidBorderBottom = Math.round(outputHeight * 0.185);
    // Base tilt ~3 degrees. Each card multiplies it by an entry from
    // POLAROID_TILT_PATTERN, picked by globalIndex -- a mix of left, right,
    // and dead-straight (the 0s), so a run of cards looks hand-placed
    // rather than stamped. Deterministic by index so a resumed render
    // chunk reproduces the same tilt for the same slot.
    const POLAROID_TILT_RAD = 0.052;
    const POLAROID_TILT_PATTERN = [0, 1, -0.7, 0, -1, 0.55, 0, 0.85, -0.45, 0.7];
    // Caption wraps tighter than the full-frame default -- it has to fit
    // inside the card, which is much narrower than the frame for a portrait
    // photo. The roast line is already capped ~80 chars upstream, so this
    // lands at two lines, occasionally three.
    const POLAROID_CAPTION_WRAP = 30;
    // Backdrop blur runs at quarter resolution then scales back up -- a
    // gblur at full 4K per photo (plus the earlier per-photo drop-shadow
    // gblur) made a real keepsake render ~50x slower than realtime,
    // nowhere near finishing inside the CI budget. Blurring 1/16 the
    // pixels is visually identical once it's this soft and dimmed.
    const polaroidBgW = Math.max(2, Math.round(outputWidth / 4));
    const polaroidBgH = Math.max(2, Math.round(outputHeight / 4));
    ...
    const photoFilters = imagePaths.map((_, i) => {
      ...
      //   "black"    -- plain letterbox/pillarbox bars. Cheaper (no split,
      //                 no second full-res scale, no gblur).
      //   "polaroid" -- the photo insets on a white card (a thicker white
      //                 strip along the bottom, like a real polaroid's
      //                 caption strip), centered over a blurred + dimmed
      //                 copy of the SAME photo. A portrait photo in a
      //                 landscape frame leaves a lot of margin no matter
      //                 what; filling it with the photo's own colours reads
      //                 as intentional where plain black read as wasted
      //                 space. Same cost profile as "blur" (it IS a blur
      //                 backdrop, plus the card pads).
      let chain;
      if (photoBackground === "black") {
        chain =
      ...
      } else if (photoBackground === "polaroid") {
        // A cream polaroid card -- tilted (POLAROID_TILT_PATTERN: some
        // left, some right, some dead straight) -- held static and fully
        // in frame over a blurred+dimmed copy of the SAME photo. No Ken
        // Burns zoom on this mode: a zoom on the whole composite drags the
        // card, and its baked-in caption strip, off the edge; the
        // crossfades between cards carry the motion instead. The roast
        // line is written INTO the bottom strip here, before the tilt, so
        // it tilts with the card -- which is why the later over-photo
        // caption block skips "polaroid", and why the zoom section below
        // leaves polaroid alone. (An earlier version also gave the card a
        // gblur drop shadow -- dropped for render cost, and barely visible
        // in motion anyway.)
        const cardMaxW = Math.round(outputWidth * 0.6);
        const tiltMul = POLAROID_TILT_PATTERN[globalIndex % POLAROID_TILT_PATTERN.length];
        const tilt = (tiltMul * POLAROID_TILT_RAD).toFixed(5);
        // Straight cards (tiltMul 0 -- ~a third of the set) skip the rgba
        // conversion and rotate entirely: the card is already opaque, just
        // overlay it. rotate on a card-sized rgba region per photo is the
        // dominant cost now that the backdrop blur is cheap.
        const tiltFilter = tiltMul === 0 ? "" : `,format=rgba,rotate=${tilt}:c=#00000000:ow=rotw(${tilt}):oh=roth(${tilt})`;

        let strip = "";
        if (roastLines && roastLines[i]) {
          const capFont = Math.round(30 * textScale);
          const capFile = path.join(outputDir, `roast-line-${globalIndex}.txt`);
          fs.writeFileSync(capFile, wrapText(roastLines[i], POLAROID_CAPTION_WRAP));
          // Centered in the bottom strip (h - borderBottom/2), dark ink on
          // the cream card, no box -- it's on white, it doesn't need one.
          strip =
            `,drawtext=fontfile=${toFilterPath(POLAROID_CAPTION_FONT_PATH)}:textfile=${toFilterPath(capFile)}:reload=0:` +
            `fontcolor=0x2E2A25:fontsize=${capFont}:line_spacing=${Math.round(6 * textScale)}:` +
            `x=(w-text_w)/2:y=h-${polaroidBorderBottom}/2-text_h/2`;
        }

        chain =
          `[${i}:v]split=2[bg${i}src][fg${i}src];` +
          `[bg${i}src]scale=${polaroidBgW}:${polaroidBgH}:force_original_aspect_ratio=increase,` +
          `crop=${polaroidBgW}:${polaroidBgH},gblur=sigma=7,eq=brightness=-0.17:saturation=0.88,` +
          `scale=${outputWidth}:${outputHeight}[bg${i}];` +
          `[fg${i}src]scale=${cardMaxW}:${polaroidCardMaxH}:force_original_aspect_ratio=decrease,` +
          `pad=iw+2*${polaroidBorder}:ih+${polaroidBorder}+${polaroidBorderBottom}:${polaroidBorder}:${polaroidBorder}:color=0xF7F3E9` +
          `${strip}${tiltFilter}[card${i}];` +
          `[bg${i}][card${i}]overlay=(W-w)/2:(H-h)/2`;
      } else {
        chain =
          `[${i}:v]split=2[bg${i}src][fg${i}src];` +
      ...
      // heroZoomIndex's punch-in takes priority over kenBurns' gentler
      // continuous drift if a slot somehow qualified for both -- see the
      // kenBurns comment above for why that's not actually a case any
      // caller hits today. Compared against globalIndex, not i --
      // heroZoomIndex is defined against the caller's full imagePaths, not
      // this range's local slice.
      const zoomRate = globalIndex === heroZoomIndex ? HERO_ZOOM_RATE_PER_SECOND : kenBurns ? KEN_BURNS_RATE_PER_SECOND : null;
      if (photoBackground === "polaroid") {
        // The polaroid branch already animates (the blurred backdrop
        // drifts, the card stays put). Just hold + trim to the slot length
        // -- no whole-frame zoom, which would drag the card and its
        // baked-in caption strip off the edge.
        chain += `,fps=25,trim=duration=${duration},setpts=PTS-STARTPTS`;
      } else if (zoomRate !== null) {
        // A rate, not a fixed end zoom -- see KEN_BURNS_RATE_PER_SECOND
        // above for why: it keeps the zoom's visible SPEED the same on
        // every slot no matter how long that particular slot is on screen
        // (roast-captioned photos run much longer than title cards/plain
        // photos). Each slot still converges on its own end zoom, just one
        // proportional to its own duration instead of a shared constant.
        const zoomTarget = Math.min(1 + zoomRate * duration, KEN_BURNS_MAX_ZOOM);
        ...
        // makes that 1px snap 0.5 of an output pixel, fully below the
        // visible threshold -- and zoompan's own s=WxH downscale mops up
        // the rest. This was dialed back to 1.5x once (4x the intermediate
        // pixels vs. 2.25x costs real encode time on a 4K slideshow) to
        // ease render cost, but the residual jitter that traded in turned
        // out to still be visible on real deliveries -- reverted back to
        // 2x, prioritizing zoom quality over that render-time saving.
        const osW = Math.round(outputWidth * 2);
        const osH = Math.round(outputHeight * 2);
        chain +=
          `,scale=${osW}:${osH}:flags=lanczos` +
      ...
      // "polaroid" writes the roast line into the card's bottom strip
      // itself (see that branch above), so this over-the-photo caption box
      // is only for "blur"/"black", where it sitting over the bottom of
      // the photo is the whole design (same as any Instagram caption bar).
      if (roastLines && roastLines[i] && photoBackground !== "polaroid") {
        const roastFontSize = Math.round(52 * textScale);
        const roastBoxBorderW = Math.round(24 * textScale);
        ...
        // original small fixed offset otherwise.
        ...
        chain +=
          ...
          `box=1:boxcolor=black@0.55:boxborderw=${roastBoxBorderW}`;
      }
    ...
    command
      ...
      .outputOptions([
        ...
        "-pix_fmt", "yuv420p",
        "-c:v", "libx264",
        // "fast": a middle ground. "slow"/crf 16 is near-lossless but an
        // 80-photo full video alone couldn't finish the 75-minute CI job at
        // it. "veryfast"'s weak motion search makes fine detail "boil"
        // frame-to-frame on the slow Ken Burns push-in -- visible on a crisp
        // 4K photo. "fast" still runs real motion estimation + psy-rd (the
        // things that fix the boil) while being ~1.5x quicker than "medium".
...
// needs it.
// Real duration of an audio file in seconds -- lib/musicTrack.js's
// buildMusicPlaylist needs each candidate track's actual length (not an
// estimate) to know exactly how many plays, and of which tracks, add up to
// covering a long full video, repeating the pool if one full pass still
// isn't enough.
...
// Joins several distinct tracks into one audio file, in the given order --
// the long-video medley case (see buildMusicPlaylist in lib/musicTrack.js)
// feeds its result in here rather than mergeFullVideoChunks' normal single
// musicPath, since -stream_loop -1 only ever repeats ONE input, not a
// sequence of different ones. Re-encodes (not -c:a copy) because the CC0
// source tracks come from different albums/encodes with no guarantee they
// share a sample rate -- stream-copying mismatched inputs through the
// concat demuxer is exactly the kind of thing that plays back with clicks
// or drifts out of sync at the join, and this filter path decodes and
// re-encodes every input consistently instead.
function concatAudioTracks(trackPaths, outputPath) {
  const outputDir = path.dirname(outputPath);
  const listPath = path.join(outputDir, `audio-concat-list-${Date.now()}.txt`);
  fs.writeFileSync(listPath, trackPaths.map((p) => `file '${path.resolve(p).replace(/'/g, "'\\''")}'`).join("\n"));

  const command = ffmpeg();
  command.input(listPath).inputOptions(["-f", "concat", "-safe", "0"]);
...
async function renderChunkedSlideshow(imagePaths, clipPaths, outputPath, musicPath, roastLines, baseSlotSeconds, styleConfig) {
...

  {
    ...
    mergeCommand
      ...
      .outputOptions(["-map", "[vbar]", "-pix_fmt", "yuv420p", "-c:v", "libx264", "-preset", "fast", "-crf", "20", "-r", "25"]);
  }
...
// Appends a brief (2s) "recappedforyou.com" sign-off to a finished social
// cut -- deliberately NOT a return of the removed intro/outro title cards
// (those spent several full seconds of an already-short cut on pure
// decoration; this is a fraction of that, and exists for a different
// reason). A social cut is the one deliverable actually meant to be
// reshared publicly on Reels/TikTok/Shorts, so it's the one place in this
// pipeline that should point back to where it came from even after the
// cut leaves the gallery entirely.
//
// Builds its own backdrop directly from lastPhotoPath (blur+darken via
// plain ffmpeg filters) rather than reusing lib/card-background.js's
// buildCardBackground -- that helper is sized for the landscape full
// video's own intro/outro cards (1920x1080) and relies on the caller
// (assembleSlideshow) to re-crop it to whatever shape is needed; doing
// the scale/crop/blur/darken/text in one filter chain here avoids a
// second sharp round-trip just to end up re-cropping it anyway.
//
// hasAudio must match whether cutPath itself has an audio stream (see
// assembleSlideshow's own includeAudio/musicPath gating in
// renderSlotRange) -- concatenating a silent-audio end card onto a
// video-only cut (a socialNoMusic booking) would otherwise hand ffmpeg's
// concat demuxer two segments with mismatched stream layouts. cutPath and
// outputPath must sit in the same directory -- see mergeFullVideoChunks's
// identical constraint, for the same reason (the concat list resolves
// bare filenames against its own directory).
const END_CARD_SECONDS = 2;

async function appendSocialEndCard(cutPath, lastPhotoPath, outputPath, { width, height, hasAudio }) {
  const fontPath = toFilterPath(FONT_PATH);
  const outputDir = path.dirname(outputPath);
  const endCardPath = path.join(outputDir, `end-card-${Date.now()}.mp4`);

  await new Promise((resolve, reject) => {
    const cmd = ffmpeg().input(lastPhotoPath).inputOptions(["-loop", "1"]);
    if (hasAudio) cmd.input("anullsrc=channel_layout=stereo:sample_rate=44100").inputOptions(["-f", "lavfi"]);
    cmd.duration(END_CARD_SECONDS).videoFilters([
      `scale=${width}:${height}:force_original_aspect_ratio=increase`,
      `crop=${width}:${height}`,
      "eq=brightness=-0.28",
      "gblur=sigma=20",
      `drawtext=fontfile=${fontPath}:text='recappedforyou.com':fontsize=46:fontcolor=white:x=(w-text_w)/2:y=(h-text_h)/2`,
    ]);
    const outputOpts = ["-pix_fmt", "yuv420p", "-c:v", "libx264", "-preset", "fast", "-crf", "20"];
    if (hasAudio) outputOpts.push("-map", "0:v", "-map", "1:a", "-c:a", "aac", "-b:a", "128k", "-shortest");
    cmd.outputOptions(outputOpts).output(endCardPath).on("end", resolve).on("error", reject).run();
  });

  const concatListPath = path.join(outputDir, `end-card-concat-${Date.now()}.txt`);
  fs.writeFileSync(concatListPath, [cutPath, endCardPath].map((p) => `file '${path.basename(p)}'`).join("\n"));

  await new Promise((resolve, reject) => {
    ffmpeg()
      .input(concatListPath)
      .inputOptions(["-f", "concat", "-safe", "0"])
      .outputOptions(["-pix_fmt", "yuv420p", "-c:v", "libx264", "-preset", "fast", "-crf", "20", ...(hasAudio ? ["-c:a", "aac", "-b:a", "128k"] : [])])
      .output(outputPath)
      .on("end", resolve)
      .on("error", reject)
      .run();
