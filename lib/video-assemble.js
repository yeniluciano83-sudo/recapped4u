  ...
  return new Promise((resolve, reject) => {
    ...
    const photoFilters = imagePaths.map((_, i) => {
      ...
      if (zoomRate !== null) {
        ...
        // blurred moving background). Feeding zoompan a frame scaled up 1.5x
        // makes that 1px snap ~0.67 of an output pixel -- enough to take the
        // jitter below the visible threshold -- and zoompan's own s=WxH
        // downscale mops up the rest. 1.5x (not 2x) keeps the intermediate
        // frame ~2.25x the pixels rather than 4x, which matters for encode
        // time on a 4K slideshow; chunked rendering keeps only a few slots
        // in flight regardless.
        const osW = Math.round(outputWidth * 1.5);
        const osH = Math.round(outputHeight * 1.5);
        chain +=
