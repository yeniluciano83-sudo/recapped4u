  ...
  return new Promise((resolve, reject) => {
    ...
    const photoFilters = imagePaths.map((_, i) => {
      ...
      if (zoomTarget !== null) {
        ...
        // above rules out -- and centering is actually safe here
    ...
    command
      ...
      .outputOptions([
        ...
        "-pix_fmt", "yuv420p",
        "-c:v", "libx264",
        // preset/crf, not resolution, is the lever here: output stays 4K
        // (OUTPUT_WIDTH/HEIGHT). "slow"/crf 16 is near-lossless and ~5-6x
        // slower than "veryfast" -- for a mostly-static slideshow (gentle
        // zoompan, no real motion) that quality is invisible, but the cost
        // is not: a Luxe booking renders a full video + a no-roast full
        // video + up to 10 social-cut pairs in one CI job, and at "slow"
        // an 80-photo booking's full video alone didn't finish inside the
        // 75-minute job timeout (see .github/workflows/recap-scheduler.yml).
        // MAX_SLOTS_PER_CHUNK bounds memory per ffmpeg process but does
        // nothing for total encode time -- same frame count either way --
        // so wall-clock has to come off the encoder settings. crf 20 is
        // still a high-quality target; files are somewhat larger than at
        // crf 16 but encode far faster.
        "-preset", "veryfast",
        "-crf", "20",
        "-r", "25",
      ]);
...
async function renderChunkedSlideshow(imagePaths, clipPaths, outputPath, musicPath, roastLines, baseSlotSeconds, styleConfig) {
  ...
  while (start < slotCount) {
    ...
    const imageEnd = Math.min(end, imagePaths.length);
    const clipStart = Math.max(0, start - imagePaths.length);
    const clipEnd = Math.max(0, end - imagePaths.length);

    const chunkImagePaths = imagePaths.slice(start, imageEnd);
    const chunkClipPaths = clipPaths.slice(clipStart, clipEnd);
    const chunkRoastLines = roastLines ? roastLines.slice(start, imageEnd) : null;
    const chunkOverlayLines = overlayLines ? overlayLines.slice(start, imageEnd) : null;
    const chunkOutputPath = path.join(outputDir, `chunk-${chunkIndex}-${Date.now()}.mp4`);
    ...
    // this whole mechanism exists to stay under.
    await renderSlotRange(chunkImagePaths, chunkClipPaths, chunkRoastLines, chunkOverlayLines, chunkOutputPath, {
      baseSlotSeconds,
      styleConfig,
      globalOffset: start,
      includeAudio: false,
  ...
  // Concat demuxer -- every chunk was encoded with the same codec/profile/
  // fps/resolution (renderSlotRange's fixed output options), so a plain
  // stream copy across them is valid and effectively free. A relative path
  // *inside* the list file is resolved by ffmpeg relative to the list
  // file's own directory, not process.cwd() -- confirmed live ("[concat]
  // Impossible to open ...", the cwd-relative toFilterPath path wrongly
  // re-based against this directory instead). Since every chunk file lives
  // in this same outputDir as the list file itself, a bare filename sidesteps
  // the ambiguity entirely rather than computing a relative path at all.
  ...
  // Unlike -filter_complex_script's argument, a plain -i path isn't
  // filtergraph syntax -- the colon-collision toFilterPath exists to dodge
  // (see its own comment) doesn't apply here, and passing it through
  // toFilterPath anyway (confirmed live) breaks fluent-ffmpeg's own path
  // handling for this argument specifically. The absolute path works fine.
  ...
  if (isVertical && slotCount > 1) {
    // Story-style progress bar -- see renderSlotRange's includeProgressBar
    // block for what this draws; only ever reachable here for a vertical
    // booking large enough to chunk, which no real caller hits today
    // (social cuts cap at 15 slots, well under MAX_SLOTS_PER_CHUNK) --
    // implemented for correctness/future-proofing rather than a case in
    // production. Segment timing is computed from the flat single-pass
    // formula, so on a hard-cut merge its last segment(s) land slightly
    // short of the true (now longer) end of the video -- the same minor
    // drift noted above, harmless for a progress bar's purpose.
    let cumulative = slotDurations[0];
    const segmentStarts = [0];
    ...
    mergeCommand
      ...
      // burn in the progress bar, so it carries the same wall-clock cost
      // and the same reasoning for keeping the encoder fast.
      .outputOptions(["-map", "[vbar]", "-pix_fmt", "yuv420p", "-c:v", "libx264", "-preset", "veryfast", "-crf", "20", "-r", "25"]);
  } else {
    // The common case (landscape full video) -- no filtering needed at
    // all, so the video stream is a pure copy: this pass is essentially
    // just a remux plus an audio encode, not a second real encode.
