...
// { text, position: "top-center" | "top-left", fontColor, boxColor } --
...
function assembleSlideshow(imagePaths, clipPaths, outputPath, musicPath, roastLines, baseSlotSeconds = SECONDS_PER_SLOT, styleConfig = {}) {
  const { transitionType = "fade", transitionSeconds = TRANSITION_SECONDS, grain = false, overlayLines = null } = styleConfig;
  return new Promise((resolve, reject) => {
    ...
    const photoFilters = imagePaths.map((_, i) => {
      ...
      let chain =
        ...
        `[bg${i}src]scale=${OUTPUT_WIDTH}:${OUTPUT_HEIGHT}:force_original_aspect_ratio=increase,` +
        `crop=${OUTPUT_WIDTH}:${OUTPUT_HEIGHT},gblur=sigma=25,eq=brightness=-0.12[bg${i}];` +
        `[fg${i}src]scale=${OUTPUT_WIDTH}:${OUTPUT_HEIGHT}:force_original_aspect_ratio=decrease[fg${i}];` +
      ...
      if (overlay) {
        ...
        fs.writeFileSync(overlayFilePath, wrapText(overlay.text));
        const xy = overlay.position === "top-left"
          ? `x=${Math.round(60 * TEXT_SCALE)}:y=${Math.round(70 * TEXT_SCALE)}`
          : `x=(w-text_w)/2:y=${Math.round(70 * TEXT_SCALE)}`; // "top-center" default
        chain +=
          `,drawtext=fontfile=${fontPath}:textfile=${toFilterPath(overlayFilePath)}:reload=0:` +
          `fontcolor=${overlay.fontColor || "white"}:fontsize=${Math.round(46 * TEXT_SCALE)}:line_spacing=${Math.round(8 * TEXT_SCALE)}:${xy}:` +
          `box=1:boxcolor=${overlay.boxColor || "black@0.6"}:boxborderw=${Math.round(20 * TEXT_SCALE)}`;
      }
