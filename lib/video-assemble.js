...
// A short, public teaser trimmed from the start of a Spotlight/Luxe
// booking's first social cut (see scripts/auto-recap.js's
// buildAndUploadTeaser) -- meant for the HOST to post to their own
// followers, not to send to guests, which is what sets it apart from
// every other export here: it's the one clip in this pipeline that
// carries a permanent brand watermark. A stranger who sees it publicly
// has no other way back to booking their own event. Always re-encodes
// (a stream-copy trim can't also burn in text), but a few seconds of
// 1080x1920 is fast and cheap regardless of that.
const TEASER_SECONDS = 7;

function buildTeaserClip(sourceVideoPath, outputPath) {
  const fontPath = toFilterPath(FONT_PATH);
  return new Promise((resolve, reject) => {
    ffmpeg(sourceVideoPath)
      .duration(TEASER_SECONDS)
      .videoFilters([
        `drawtext=fontfile=${fontPath}:text='recappedforyou.com':fontsize=30:fontcolor=white@0.9:borderw=2:bordercolor=black@0.65:x=(w-text_w)/2:y=h-th-48`,
      ])
      .outputOptions(["-pix_fmt", "yuv420p", "-c:v", "libx264", "-preset", "fast", "-crf", "20", "-c:a", "aac", "-b:a", "128k"])
      .output(outputPath)
      .on("end", () => resolve(outputPath))
      .on("error", (err) => reject(err))
      .run();
