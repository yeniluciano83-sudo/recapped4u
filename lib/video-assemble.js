...
function renderSlotRange(imagePaths, clipPaths, roastLines, overlayLines, outputPath, opts) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const polaroidShadowDX = Math.round(outputWidth * 0.007);
    const polaroidShadowDY = Math.round(outputHeight * 0.013);
    ...
    const photoFilters = imagePaths.map((_, i) => {
      ...
      if (photoBackground === "black") {
        ...
        // left, some right, some dead straight), soft drop shadow -- held
        // static and fully in frame over a blurred+dimmed copy of the SAME
        // photo. No Ken Burns zoom on this mode: a zoom on the whole
        // composite drags the card, and its baked-in caption strip, off
        // the edge; the crossfades between cards carry the motion instead.
        // The roast line is written INTO the bottom strip here, before the
        // tilt, so it tilts with the card -- which is why the later
        // over-photo caption block skips "polaroid", and why the zoom
        // section below leaves polaroid alone.
        const cardMaxW = Math.round(outputWidth * 0.6);
        const tilt = (POLAROID_TILT_PATTERN[globalIndex % POLAROID_TILT_PATTERN.length] * POLAROID_TILT_RAD).toFixed(5);
        ...
        chain =
          ...
          `crop=${outputWidth}:${outputHeight},gblur=sigma=28,eq=brightness=-0.17:saturation=0.88[bg${i}];` +
          ...
          `${strip},format=rgba,rotate=${tilt}:c=#00000000:ow=rotw(${tilt}):oh=roth(${tilt})[card${i}];` +
          `[card${i}]split=2[card${i}a][card${i}b];` +
          `[card${i}b]colorchannelmixer=rr=0:gg=0:bb=0,gblur=sigma=24,colorchannelmixer=aa=0.4[shad${i}];` +
          `[bg${i}][shad${i}]overlay=x='(W-w)/2+${polaroidShadowDX}':y='(H-h)/2+${polaroidShadowDY}'[ws${i}];` +
          `[ws${i}][card${i}a]overlay=(W-w)/2:(H-h)/2`;
      } else {
        chain =
          `[${i}:v]split=2[bg${i}src][fg${i}src];` +
          `[bg${i}src]scale=${outputWidth}:${outputHeight}:force_original_aspect_ratio=increase,` +
