...
function assembleSlideshow(imagePaths, clipPaths, outputPath, musicPath, roastLines, baseSlotSeconds = SECONDS_PER_SLOT, styleConfig = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const photoFilters = imagePaths.map((_, i) => {
      ...
      chain +=
        `,zoompan=z=1:d=${Math.round(duration * 25)}:s=${outputWidth}x${outputHeight}:fps=25,` +
      ...
      if (roastLines && roastLines[i]) {
        ...
        chain +=
          ...
          `fontcolor=white:fontsize=${Math.round(52 * textScale)}:line_spacing=${Math.round(10 * textScale)}:x=(w-text_w)/2:y=h-text_h-${Math.round(70 * textScale)}:` +
      ...
      if (overlay) {
        ...
        chain +=
          ...
          `fontcolor=${overlay.fontColor || "white"}:fontsize=${Math.round(46 * textScale)}:line_spacing=${Math.round(8 * textScale)}:${xy}:` +
